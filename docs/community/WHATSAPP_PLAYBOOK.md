# SwimBuddz Community WhatsApp Playbook

_Last updated: 2026-05-04_

## Purpose

WhatsApp is the heartbeat of the SwimBuddz community. It's where members connect between sessions, where prospective members get a feel for the culture, and where the community stays alive even when nobody's in the water. This playbook defines the group structure, weekly rhythm, content formats, and moderation approach.

---

## Group Structure (WhatsApp Community)

SwimBuddz uses a **WhatsApp Community** (WhatsApp's feature that groups related chats under one parent). This separates audiences by intent while keeping everything under one roof.

### Groups inside the Community

| Group                                                    | Who's in it                                                                                                    | Purpose                                                                                                                                                                                                                             |
| -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **SwimBuddz - New Here?**                                | Prospective members who found us via social, referral, or the site's link-in-bio                               | Low-pressure entry point. People introduce themselves, ask questions, observe the culture before committing. This is the WA group linked from [swimbuddz.com/links](<../../swimbuddz-frontend/src/app/(public)/links/page.tsx>).    |
| **SwimBuddz Community**                                  | All active members — community, club, and academy                                                              | Main cross-layer group. Spotlights, question of the week, session recaps, general swim chat.                                                                                                                                        |
| **SwimBuddz Volunteers**                                 | All registered volunteers, across every role in [VOLUNTEER_ROLES.md](./VOLUNTEER_ROLES.md) (Tier 1, 2, 3)      | The operational home for volunteering. Weekly opportunity posts, sign-ups, swaps, last-minute cover, tier milestones, Volunteer of the Month. Kept focused on volunteering — general chat stays in the main Community group.        |
| **Pool Ambassadors**                                     | Members actively rostered as Pool Ambassadors (see [POOL_AMBASSADOR_PROGRAM.md](./POOL_AMBASSADOR_PROGRAM.md)) | Sub-group for the ambassador rotation. Pre-session briefings, conversion tracking reports, follow-up message templates, shared learnings on what worked poolside. Members rotate in/out as they take up or step back from the role. |
| **Club pod groups** (one per pod, e.g. `SB Club – Dolphins`)  | The 5 Club members in that pod                                                                                 | The pod's home: Saturday logistics, midweek "anyone in for a swim?" coordination, photos/videos/PBs, the weekly challenge. Run by the Pod Lead. See [Pod Operations](../club/POD_OPERATIONS.md).                                    |
| **Academy cohort groups** (one per live cohort)          | Students + coach for that 12-week cohort                                                                       | Cohort-specific coordination — session reminders, skill videos, questions about that week's theme. Archived when the cohort ends.                                                                                                   |
| **SwimBuddz team** (admin-only)                          | Founder, community manager, coaches                                                                            | Internal coordination, not for members.                                                                                                                                                                                             |

**Community invite link (used in IG/TikTok bio via `/links`):** the "SwimBuddz - New Here?" group. Prospects land here first; they're moved or invited into the main Community group once they sign up for Academy, Club, or buy a Community membership.

### Group descriptions

- **SwimBuddz - New Here?** — "New to SwimBuddz? Say hi 👋 Ask anything. We'll point you to the right programme. swimbuddz.com/links"
- **SwimBuddz Community** — "Lagos' swimming community. Sessions, events, conversations, and everything swimming. swimbuddz.com"
- **SwimBuddz Volunteers** — "The people who make SwimBuddz run. Opportunities, sign-ups, and recognition. Roles guide: swimbuddz.com/guides/volunteer-roles"
- **Pool Ambassadors** — "Active ambassador rotation. Briefings, follow-ups, conversion tracking."
- **Club pod groups** — Use the standard pod description below (paste verbatim, fill in the bracketed bits). The Pod Lead also sends the [pod kickoff message](../club/POD_LEAD_GUIDE.md#pod-kickoff-message) within 24 hours of group creation.

  ```
  🐬 SB Club – [Pod name]
  Your swim crew. Same 5, every Saturday — and whenever else we can get in the water together.

  🏊 Saturday [time + pool] (default — your lead may move it if it can't hold)
  📍 Lead: [Pod Lead name]
  ✅ Sign in via the app for the Saturday session — attendance counts

  House rules
  • Be kind. Effort > performance.
  • No spam, promos, or forwards.
  • Coaching qs → next academy cohort. Money/schedule → SwimBuddz team.

  Wider chat → main SwimBuddz group. swimbuddz.com
  ```

  Pin a longer welcome message in the group covering Saturday default + reschedule rule, midweek "anyone in?" coordination, photo/video posting, and the ad-hoc-swims-don't-count-toward-attendance rule. Template lives in [Pod Operations](../club/POD_OPERATIONS.md#between-saturdays).
- **Academy cohort groups** — "[Cohort name] • [Weeks X–Y] • Coach [Name]"

### Admins

- **Founder** — admin on every group
- **Community manager / designated moderator** — admin on New Here?, main Community, Volunteers, Pool Ambassadors, and team group
- **Volunteer coordinator (if appointed)** — admin on Volunteers and Pool Ambassadors
- **Pod Leads** — admin on their own pod group; post permissions (not full admin) on the main Community
- **Coaches** — admin on their own active cohort groups

### Flow between groups

```
Instagram/TikTok bio → swimbuddz.com/links → "New Here?" group
                                                    ↓
                               signs up for Academy / Club / Community
                                                    ↓
                                       added to "SwimBuddz Community"
                                              + relevant cohort or
                                              Club pod group
                                                    ↓
                               (optional) registers as a volunteer
                                                    ↓
                                    added to "SwimBuddz Volunteers"
                                                    ↓
                            (if they take up the ambassador role)
                                                    ↓
                                    added to "Pool Ambassadors"
```

Nobody is in "New Here?" forever. Once they convert (or if they go quiet for 6+ weeks), they're moved out. Keeps the group fresh for actual new arrivals.

---

## Weekly Rhythm

The group has two anchor posts per week that create predictable conversation peaks. Everything else is organic.

| Day                | Content                                             | Owner                  |
| ------------------ | --------------------------------------------------- | ---------------------- |
| **Monday AM**      | Question of the Week                                | Community manager      |
| **Wednesday**      | Volunteer opportunity post (when available)         | SwimBuddz team         |
| **Thursday PM**    | Member Spotlight of the Week                        | Community manager      |
| **Friday**         | Session booking reminder (before 3pm deadline)      | Automated or team      |
| **Saturday**       | Post-session energy — photos, results, celebrations | Pod Leads / members    |
| **Sunday–Tuesday** | Organic conversation, pool tips, gear chat          | Everyone               |

### Thursday: Member Spotlight (2:00 PM)

The spotlight is the signature community ritual. One member featured per week.

**Preparation (Sunday/Monday before):**
Message the featured member privately:

> "Hey [Name]! You're our SwimBuddz Spotlight Member this week 🏊 Can you answer these 5 quick questions and send me a photo of you (swimming or not, your choice)? I'll post it Thursday afternoon."

Send them the 5 questions (see question bank below). They have until Wednesday to respond.

**Post format:**

```
🏊 *SwimBuddz Member Spotlight*
_This week: [First Name]_

📸 [Photo]

*5 Quick Facts:*
1️⃣ [Question]: [Their answer]
2️⃣ [Question]: [Their answer]
3️⃣ [Question]: [Their answer]
4️⃣ [Question]: [Their answer]
5️⃣ [Question]: [Their answer]

💬 Drop a comment if you know [Name] or want to say hello! 👋
```

**After posting:**

- React to the post yourself with an emoji
- Tag 2-3 members who have a connection to the featured person: "@Temi you also swim at Federal Palace right?"
- Reply to every comment in the first hour to keep momentum going
- If conversation dies, ask a follow-up question about one of their answers
- **Friday:** cross-post to the **SwimBuddz - New Here?** group so prospects see the spotlight *with* the comment activity already on it (see [New Here? Group Operating Rules — Rule 3](#rule-3-spotlight-cross-post-timing))

**Selection order:**

- Weeks 1-4: Pick your most outgoing, interesting members. They set the tone.
- Weeks 5+: Mix of established and newer members. Everyone should want to be featured.
- Always have 3-4 spotlights pre-collected so you never miss a Thursday.

### Monday: Question of the Week (9:00 AM)

A single question thrown to the entire group. No featured member — everyone answers.

**Post format:**

```
🤔 *Question of the Week*

[Question here]

Drop your answer below 👇
```

**After posting:**

- Answer the question yourself first (sets the tone, shows it's safe to share)
- React to every response with an emoji
- Create connections: "Oh interesting — @Chidi said something similar!"
- Pin the post so latecomers see it

---

## Spotlight Question Bank

Each spotlight uses 4 fixed questions + 1 rotating wildcard. This keeps them comparable while adding variety.

### Fixed Questions (always use these 4):

1. What do you do during the week?
2. How did you start swimming / what brought you to SwimBuddz?
3. What's your favourite pool in Lagos?
4. What's one thing swimming has changed for you?

### Wildcard Questions (rotate one per week):

1. What's your go-to post-swim food?
2. What song gets you hyped before a swim?
3. If you could swim anywhere in the world, where?
4. What's something people wouldn't guess about you?
5. What did Saturday mornings look like before SwimBuddz?
6. What's the best trip you've ever taken?
7. If SwimBuddz went on a group trip, where should we go?
8. What's a skill you'd love to learn next (outside of swimming)?
9. What's the funniest thing that's happened to you at a pool?
10. What would you tell someone who's scared to start swimming?
11. Morning swim or evening swim — and why?
12. What's your swimming goal for this year?

---

## Question of the Week Bank

Rotate through these. Mix swimming-related and personal questions to keep it fresh.

### Swimming & Fitness

1. What's the hardest part of swimming for you right now?
2. Anyone tried a new pool recently? How was it?
3. What's your swimming goal for this month?
4. How do you warm up before getting in the water?
5. What piece of swim gear made the biggest difference for you?
6. What stroke do you want to improve most?
7. Do you swim better in the morning or evening?
8. What's the longest distance you've swum without stopping?
9. How do you stay motivated to keep swimming?
10. What's the best swimming advice you've ever received?

### Lifestyle & Fun

11. What's everyone doing this weekend (besides swimming)?
12. What's on your playlist right now?
13. Best restaurant/spot you've been to recently in Lagos?
14. What's one habit you've picked up this year?
15. If you could have dinner with anyone, who?
16. What's the best purchase you've made under N5,000?
17. What book/show/podcast are you into right now?
18. What's your unpopular opinion about fitness?
19. Early bird or night owl?
20. What's one thing on your bucket list?

### Community & Connection

21. What's something you wish more people in this group knew about you?
22. What's one thing SwimBuddz could do better?
23. Would you be interested in [specific event idea]? (Use for gauging interest)
24. What brought you to this group?
25. If you could add one thing to our sessions, what would it be?

---

## Moderation Guidelines

### Tone

The group should feel like a friend's living room — welcoming, energetic, occasionally silly, never corporate. SwimBuddz team posts should sound like a person, not a brand.

### Rules (pin these in the group description)

```
SwimBuddz Community Rules:
1. Be kind. We're all here because we love swimming.
2. No spam, promotions, or forwarded broadcasts.
3. Keep it relevant — swimming, fitness, community, Lagos life.
4. Respect everyone's swimming level. No shaming.
5. Questions are encouraged. No stupid questions here.
6. Admin reserves the right to remove disruptive members.
```

### What to moderate:

- **Spam / promotions:** Remove immediately. One warning, then remove from group.
- **Off-topic chains that go too long:** Let them run for a bit (organic conversation is good), but if it drowns out community content, gently redirect: "Love this conversation! Let's get back to swimming — anyone have a pool recommendation for the mainland?"
- **Negativity / complaints:** Don't delete unless it's abusive. Acknowledge publicly ("That's fair feedback, thanks for sharing"), then take the conversation to DM if it needs resolution.
- **Silence:** If the group goes quiet for 2+ days, drop a casual prompt: "Who's swimming this weekend?" or share a swimming tip.

### What NOT to moderate:

- Off-topic conversations (within reason) — this is how relationships form
- Members recommending things to each other (restaurants, gear, etc.)
- Banter and jokes — if people are laughing, the community is alive
- Members organising informal swims outside of official sessions

### Handling prospective members:

- When someone new joins, welcome them personally: "Welcome [Name]! Great to have you here. Feel free to ask anything — we're all friendly."
- Don't hard-sell. Let them observe the community energy. The spotlight posts, question threads, and session recaps do the selling.
- If they ask about joining, give them the clear tier breakdown (see PRICING_STRATEGY.md) and point them to the website.

---

## Volunteer Group Operating Rules

The **SwimBuddz Volunteers** group follows the same tone and moderation rules as the main Community group, plus three rules specific to how it's run.

### Rule 1: Operational, not social

The Volunteers group exists to run the volunteering function. Keep it focused:

**Belongs here:**

- Opportunity posts (Wednesday opportunity, last-minute cover requests)
- Sign-ups, swaps, cancellations
- Role briefings and debriefs
- Tier milestones, Volunteer of the Month, session-specific thanks
- Questions about roles, tiers, reliability scores, perks
- Links to [VOLUNTEER_ROLES.md](./VOLUNTEER_ROLES.md) and operational docs

**Doesn't belong here — redirect to the main Community group:**

- General swim chat, gear talk, Lagos life
- Spotlights and Question of the Week
- Session recap photos (unless they specifically celebrate volunteers)

If a conversation starts drifting, gently redirect: "Love this — let's take it to the main group so everyone catches it. Back here for volunteer stuff 🙏"

**Why:** This group has to stay scannable. When a volunteer opens it on Saturday morning, they need to see the opportunity, not 40 messages of off-topic chat. Operational groups die when they become social groups — people mute them and miss the sign-ups.

### Rule 2: Tier progression shout-outs

Volunteer tiers and recognition levels are defined in [VOLUNTEER_ROLES.md](./VOLUNTEER_ROLES.md#recognition). Celebrate them publicly in the Volunteers group when members cross a threshold:

| Trigger                   | Post                                                                                                                         |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| Hits Bronze (10+ hours)   | "🥉 [Name] just hit Bronze — 10+ volunteer hours. Priority access to community events unlocked. Thank you for showing up 🙏" |
| Hits Silver (50+ hours)   | "🥈 [Name] just hit Silver — 50+ hours. 20% off session fees earned. This is what consistency looks like 👏"                 |
| Hits Gold (100+ hours)    | "🥇 [Name] reached Gold — 100+ hours volunteered. 50% off membership. Legends of SwimBuddz 🏅"                               |
| Promoted to Tier 2 (Core) | "🔆 [Name] is now a Core volunteer. 20+ hours, 80+ reliability. Can now take on Session Lead, Safety, Mentor roles."         |
| Promoted to Tier 3 (Lead) | "⭐ [Name] is now a Lead volunteer. 50+ hours, 90+ reliability. The backbone."                                               |

Also post **Volunteer of the Month** after the month closes, in both the Volunteers group and the main Community group. The public site rotation is calculated from the closed month's logged volunteer hours/activity and keeps the winner featured for the next display month.

**Why:** Recognition is the reward loop. The tier system only works if people see others being recognised — otherwise Bronze/Silver/Gold are just numbers in a doc. Public shout-outs also create aspiration: newer volunteers see what's possible.

### Rule 3: Inactive-volunteer check-in policy

Volunteers who have not signed up for or completed any volunteering action in **8 weeks** get a gentle DM from the community manager, not auto-removal.

```
Hey [Name], noticed we haven't seen you sign up for anything in a while. No pressure at all — life gets busy. Just checking in:
— Still want to be in the Volunteers group? (happy either way)
— Anything getting in the way we could help with?
— Any role you'd want to try next?

We're not tracking this to nag — just want to make sure the group stays useful for you.
```

**Why:**

- 8 weeks (not 6 like the "New Here?" group) because volunteers have already opted in — they deserve more patience than a cold prospect.
- A DM rather than silent removal because most inactive volunteers are dealing with something external, not disengaged from SwimBuddz. Auto-removal burns the relationship.
- Tracks reliability score more holistically — someone with 90+ reliability who's been quiet for 2 months is very different from someone who repeatedly no-shows.

If there's no response after 2 weeks, move them out of the Volunteers group but keep them in the main Community group. They can rejoin whenever they register for a role again.

---

## New Here? Group Operating Rules

The **SwimBuddz - New Here?** group is a low-pressure entry point for prospective members. Same tone and moderation rules as the main Community group, plus four rules specific to how it's run.

### Rule 1: No hard-sell

This is the prospect's *observation* phase, not the conversion call. Things that don't belong here:

- Cohort sales pitches with prices and urgency framing ("first come, first swim")
- "You need to register to be a member"-style nudges from team or volunteers
- Repeated CTAs to sign up

Cohort and academy promos belong in the **main Community group** (where members amplify them) or in **DMs to qualified prospects** — people who've already said they want to learn or join. Brief any volunteer or team member helping in this group on this rule; gently correct if they slip.

**The CTA that works here:** "We're at [pool] this Saturday — come watch a session and meet the crew. No commitment." A real Saturday outperforms any pitch.

**Why:** Prospects in this group haven't opted in to being sold to — they came to *observe*. Selling too early makes the group feel transactional and prospects mute or leave. The cohort pitch lands when they're ready: at the pool, in DM, or in the main group after they've converted.

### Rule 2: Show, don't tell — let members do the talking

The energy comes from **members being members**, not the founder posting more. Use these content types to give prospects glimpses of the real community:

| Content                       | What it is                                                                                                                                                       | Cadence                  |
| ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------ |
| **Weekend session recap**     | 2-3 photos from Saturday + one short member quote ("first time swimming 50m without stopping — wild")                                                            | Sunday or Monday         |
| **Spotlight cross-post**      | Repost the week's spotlight from the main Community group (see Rule 3 for timing)                                                                                | Friday                   |
| **Organic conversation snap** | Screenshot of a fun thread from the main group — gear debate, pool rec chain, banter — with permission, captioned "this is a normal Tuesday in here"             | Once a week              |
| **Member voice/video**        | Short clip or voice note from a member ("come swim with us this Saturday")                                                                                       | When organic, not forced |

That's 2-3 native posts per week — all glimpses, zero pressure.

### Rule 3: Spotlight cross-post timing

Spotlights drop in the **main Community group on Thursday at 2:00 PM** (per Weekly Rhythm). They cross-post to **New Here? on Friday**, *after* the comments have built up in the main group.

**Why Friday, not same-day:** prospects who scroll back see real members reacting, tagging each other, sharing connections — that comment activity is the social proof. A same-day cross-post lands without comments because prospects don't know the featured member; it reads like a brochure.

### Rule 4: Welcomes are personal, not blurb-dumps

The full welcome message should be **pinned once** in the group. Do **not** re-post the full blurb every time someone new joins — it buries everything else and trains people to scroll past.

**When someone new joins:**

```
Welcome [Name] 🙌 Pinned post has the basics.
What brought you here? And which side of Lagos are you on?
```

The pin does the explaining; you do the connecting. Two qualifying questions in one message get you the info you need to route them (cohort vs. club vs. just hang out) without an interrogation.

**Bring in member ambassadors:** invite 2-3 of your most outgoing existing members (the same ones you'd spotlight first) into New Here? as informal welcomers. When a prospect sees *another member* welcoming them — not just the founder — perceived community energy multiplies. This is the embryo of the Pool Ambassador role defined in [POOL_AMBASSADOR_PROGRAM.md](./POOL_AMBASSADOR_PROGRAM.md).

### Suggested weekly rhythm (New Here?)

| Day               | Content                                                |
| ----------------- | ------------------------------------------------------ |
| **Sat eve / Sun** | Weekend session recap (photos + member quote)          |
| **Mid-week**      | Organic conversation screenshot or member voice note   |
| **Friday**        | Spotlight cross-post (after main group comments build) |
| **Always-on**     | Personal welcomes + 1:1 routing questions              |

---

## Content Calendar Template (Monthly)

### Week 1

- **Mon:** Question: Swimming-related ("What's your swimming goal for this month?")
- **Thu:** Spotlight #1 (pick a strong personality to set the tone)

### Week 2

- **Mon:** Question: Fun/lifestyle ("Best restaurant you've been to recently in Lagos?")
- **Thu:** Spotlight #2

### Week 3

- **Mon:** Question: Community ("What's one thing SwimBuddz could do better?")
- **Thu:** Spotlight #3

### Week 4

- **Mon:** Question: Swimming-related ("What piece of swim gear made the biggest difference for you?")
- **Wed:** Volunteer of the Month announcement (if applicable)
- **Thu:** Spotlight #4

### Recurring (every week):

- **Wed:** Volunteer opportunities (when available)
- **Fri before 3pm:** Session booking reminder
- **Sat evening:** Post-session recap / photos / celebrations

---

## Metrics That Matter

Track these monthly to know if the community is healthy:

| Metric                          | Healthy        | Warning           |
| ------------------------------- | -------------- | ----------------- |
| Spotlight comments              | 5+ per post    | <3                |
| Question of the Week responses  | 8+             | <4                |
| Unique members posting per week | 10+            | <5                |
| Days with zero messages         | 0-1            | 3+                |
| New member welcome responses    | Within 2 hours | Next day or never |
| Prospective → member conversion | Track monthly  | —                 |

---

## Emergency Protocols

**If someone posts something inappropriate:**

1. Delete the message immediately
2. Send them a private message explaining why
3. If severe (harassment, abuse), remove from group without warning

**If there's a safety incident at a session:**

1. Post a brief, factual update to the group: "There was an incident at today's session. Everyone is safe. We'll share more details once we have them."
2. Do NOT let speculation run wild — pin the official update
3. Follow up with a detailed post once the situation is resolved

**If the group gets too noisy (50+ messages in an hour):**

1. Let it run if it's positive energy (post-event excitement, etc.)
2. If it's a heated debate, step in: "Great discussion! Let's cool it here and pick it up at the next session."
3. Consider creating a sub-group for specific topics if it becomes a pattern

---

## Pre-Launch Checklist

Before launching the spotlight and question of the week:

- [ ] Announce the new format in the group (see Q2 announcement template)
- [ ] Have 4 spotlights pre-collected and ready
- [ ] Have 8 questions of the week queued (2 months of Thursdays)
- [ ] Brief Pod Leads on the format so they can amplify
- [ ] Pin the group rules
- [ ] Set a weekly reminder to collect this week's spotlight (Sunday/Monday)

---

_The goal is simple: when someone opens their phone on a random Tuesday, the SwimBuddz group should be one of the chats they actually want to read._
